...
//! let other = BitSet::<u32>::from_bytes(&[0b11010000]);
//!
//! s.union_with(&other);
//!
...
impl<B: BitBlock> BitSet<B> {
    ...
    /// use bit_set::BitSet;
    ///
    /// let mut s = BitSet::<u32>::with_capacity(100);
    ...
    pub fn from_bytes(bytes: &[u8]) -> Self {
        BitSet { bit_vec: BitVec::<B>::from_bytes(bytes) }
    }
}
...
impl<B: BitBlock> BitSet<B> {
    ...
    /// use bit_set::BitSet;
    ///
    /// let s = BitSet::<u32>::from_bytes(&[0b01001010]);
    ///
    ...
    #[inline]
    pub fn dbl_iter(&self) -> IndexIter<B> {
        self.bit_vec.iter().into()
    }
    ...
    /// use bit_set::BitSet;
    ///
    /// let a = BitSet::<u32>::from_bytes(&[0b01101000]);
    /// let b = BitSet::<u32>::from_bytes(&[0b10100000]);
    ///
    ...
    /// let res = 0b11101000;
    ///
    /// let mut a = BitSet::<u32>::from_bytes(&[a]);
    /// let b = BitSet::<u32>::from_bytes(&[b]);
    /// let res = BitSet::<u32>::from_bytes(&[res]);
    ///
    /// a.union_with(&b);
    /// assert_eq!(a, res);
    /// ```
    ...
    ///
    /// let mut bva = BitSet::<u32>::from_bytes(&[a]);
    /// let bvb = BitSet::<u32>::from_bytes(&[b]);
    /// let bva_b = BitSet::<u32>::from_bytes(&[a_b]);
    /// let bvb_a = BitSet::<u32>::from_bytes(&[b_a]);
    ///
    /// bva.difference_with(&bvb);
    /// assert_eq!(bva, bva_b);
    ///
    /// let bva = BitSet::<u32>::from_bytes(&[a]);
    /// let mut bvb = BitSet::<u32>::from_bytes(&[b]);
    ///
...
/*
    ...
    /// use bit_set::BitSet;
    ///
    /// let mut a = BitSet::<u32>::new();
    /// a.insert(2);
    /// a.insert(6);
    ///
    /// let mut b = BitSet::<u32>::new();
    /// b.insert(1);
    /// b.insert(3);
    /// b.insert(6);
    ///
...
*/
    ...
    /// assert_eq!(a.len(), 2);
    /// assert_eq!(b.len(), 2);
    /// assert_eq!(a, BitSet::<u32>::from_bytes(&[0b01100000]));
    /// assert_eq!(b, BitSet::<u32>::from_bytes(&[0b00010010]));
    /// assert!(a.contains(1));
    /// assert!(a.contains(2));
    /// assert!(b.contains(6));
    /// assert!(b.contains(3));
    ...
    pub fn split_off(&mut self, at: usize) -> Self {
        ...
        unsafe {
            other.bit_vec
                .storage_mut()
                .extend(repeat(B::zero())
                    .take(w)
                    .chain(self.bit_vec.storage()[w..].iter().cloned()));

            other.bit_vec.set_len(self.bit_vec.len());

            if b > 0 {
                other.bit_vec.storage_mut()[w] &= !B::zero() << b;
            }
        }
    ...
    pub fn insert(&mut self, value: usize) -> bool {
        ...
        if value >= capacity {
            ...
            self.bit_vec.reserve(alloc_2x - capacity);
        }
...
impl<'a, B> From<bit_vec::Iter<'a, B>> for IndexIter<'a, B>
    where B: BitBlock
{
    fn from(iter: bit_vec::Iter<'a, B>) -> Self {
        IndexIter {
            i: 0,
            j: iter.len(),
            iter,
...
impl<'a, B> Iterator for IndexIter<'a, B>
    where B: BitBlock
{
    type Item = usize;
    ...
    fn next(&mut self) -> Option<usize> {
        ...
        if self.i == self.iter.len() { return None }

        match self.iter.next() {
            Some(true) => Some(self.i - 1),
...
impl<'a, B> DoubleEndedIterator for IndexIter<'a, B>
    where B: BitBlock
{
    fn next_back(&mut self) -> Option<usize> {
        while let Some(next) = self.iter.next_back() {
            ...
            if next { return Some(self.j) }
        }

        None

        // match self.iter.next_back() {
        //     Some(true) => Some(self.j),

        //     Some(false) => self.next_back(),
...
//                 None => return None
//             }
//             if let Some(o) = self.head_offset.checked_sub(B::bits()) {
//                 self.head_offset -= o;
//             } else {
//                 return None
//             }
...
//     #[inline]
//     fn next_back(&mut self) -> Option<usize> { self.0.next_back() }
// }
...
mod tests {
    ...
    #[test]
    fn verifies_dbl_ended_iter_produces_reverse_order() {
        let mut set = BitSet::<u64>::new();
        let mut r = thread_rng();

        for _ in 0..10_000 {
            set.insert(r.gen_range(0usize, 1_000_000));
        }

        let xs: Vec<_> = set.iter().collect();

        assert_eq!(set.iter().count(), set.dbl_iter().rev().count());

        let mut ys = xs.clone();
        ys.sort();

        // rev of explicitly sorted vec
        //
        let zs: Vec<_> = ys.iter().rev().cloned().collect();
        ...
        assert_eq!(zs.iter().next().cloned().unwrap(), set.dbl_iter().rev().next().unwrap());
    ...
    #[test]
    fn it_tests_whether_bitset_iter_in_order() {
        let mut set = BitSet::<u64>::new();
        let mut r = thread_rng();
    ...
    fn test_bit_set_from_usizes() {
        ...
        let a: BitSet<u32> = usizes.into_iter().collect();
        let mut b = BitSet::<u32>::new();
        b.insert(0);
        b.insert(2);
        b.insert(3);
    ...
    fn test_bit_set_iterator() {
        let usizes = vec![0, 2, 2, 3];
        let bit_vec: BitSet<u32> = usizes.into_iter().collect();

        let idxs: Vec<_> = bit_vec.iter().collect();
        assert_eq!(idxs, [0, 2, 3]);

        let long: BitSet<u32> = (0..10000).filter(|&n| n % 2 == 0).collect();
    ...
    fn test_bit_set_frombit_vec_init() {
        ...
        for &b in &bools {
            for &l in &lengths {
                let bitset = BitSet::<u32>::from_bit_vec(BitVec::from_elem(l, b));
    ...
    fn test_bit_vec_masking() {
        ...
        let mut bs = BitSet::<u32>::from_bit_vec(b);
    ...
    fn test_bit_set_subset() {
        let mut set1 = BitSet::<u32>::new();
        let mut set2 = BitSet::<u32>::new();
    ...
    fn test_bit_set_is_disjoint() {
        let a = BitSet::<u32>::from_bytes(&[0b10100010]);
        let b = BitSet::<u32>::from_bytes(&[0b01000000]);
        let c = BitSet::<u32>::new();
        let d = BitSet::<u32>::from_bytes(&[0b00110000]);
    ...
    fn test_bit_set_union_with() {
        ...
        let mut b = BitSet::<u32>::new();
        b.insert(5);
        let expected = BitSet::<u32>::from_bytes(&[0b10000100]);
        a.union_with(&b);
        assert_eq!(a, expected);

        // Standard
        let mut a = BitSet::<u32>::from_bytes(&[0b10100010]);
        let mut b = BitSet::<u32>::from_bytes(&[0b01100010]);
        let c = a.clone();
        a.union_with(&b);
        b.union_with(&c);
        assert_eq!(a.len(), 4);
    ...
    fn test_bit_set_intersect_with() {
        ...
        let mut a = BitSet::<u32>::from_bytes(&[0b10100010]);
        let mut b = BitSet::<u32>::from_bytes(&[0b00000000]);
        let c = a.clone();
        a.intersect_with(&b);
        b.intersect_with(&c);
        assert!(a.is_empty());
    ...
    fn test_bit_set_difference_with() {
        // Explicitly 0'ed bits
        let mut a = BitSet::<u32>::from_bytes(&[0b00000000]);
        let b = BitSet::<u32>::from_bytes(&[0b10100010]);
        a.difference_with(&b);
        assert!(a.is_empty());

        // Uninitialized bits should behave like 0's
        let mut a = BitSet::<u32>::new();
        let b = BitSet::<u32>::from_bytes(&[0b11111111]);
        a.difference_with(&b);
        assert!(a.is_empty());
    ...
    fn test_bit_set_symmetric_difference_with() {
        ...
        let mut a = BitSet::<u32>::from_bytes(&[0b10100010]);
        let b = BitSet::<u32>::new();
        let c = a.clone();
        a.symmetric_difference_with(&b);
        assert_eq!(a, c);

        // Standard
        let mut a = BitSet::<u32>::from_bytes(&[0b11100010]);
        let mut b = BitSet::<u32>::from_bytes(&[0b01101010]);
        let c = a.clone();
        a.symmetric_difference_with(&b);
    ...
    fn test_bit_set_eq() {
        let a = BitSet::<u32>::from_bytes(&[0b10100010]);
        let b = BitSet::<u32>::from_bytes(&[0b00000000]);
        let c = BitSet::<u32>::new();
...
*/
    ...
    fn test_bit_set_split_off() {
        // Split at 0
        let mut a = BitSet::<u32>::from_bytes(&[0b10100000, 0b00010010, 0b10010010,
        ...
        assert_eq!(b, BitSet::<u32>::from_bytes(&[0b10100000, 0b00010010, 0b10010010,
                                           0b00110011, 0b01101011, 0b10101101]));
        ...
        assert_eq!(a, BitSet::<u32>::from_bytes(&[0b10100000, 0b00010010, 0b10010010,
                                           0b00110011, 0b01101011, 0b10101101]));
        ...
        assert_eq!(b, BitSet::<u32>::from_bytes(&[0, 0, 0, 0,
...
mod bench {
    ...
    #[bench]
    fn it_uses_default_iter_to_count_n_divisible_by_7(b: &mut Bencher) {
        let mut r = rng();
        let mut set = BitSet::<u64>::new();
        for _ in 0..10_000 {
            set.insert(r.gen_range(0usize, 100_000));
        }
    ...
    #[bench]
    fn it_uses_default_iter_on_last_100(b: &mut Bencher) {
        let mut r = rng();
        let mut set = BitSet::<u64>::new();
        for _ in 0..10_000 {
            set.insert(r.gen_range(0usize, 100_000));
    ...
    #[bench]
    fn it_uses_dbl_iter_to_count_n_divisible_by_7(b: &mut Bencher) {
        let mut r = rng();
        let mut set = BitSet::<u64>::new();
        for _ in 0..10_000 {
            set.insert(r.gen_range(0usize, 100_000));
    ...
    #[bench]
    fn it_uses_dbl_iter_to_iterate_over_last_100(b: &mut Bencher) {
        let mut r = rng();
        let mut set = BitSet::<u64>::new();
        for _ in 0..10_000 {
            set.insert(r.gen_range(0usize, 100_000));
    ...
    fn bench_bit_vecset_small(b: &mut Bencher) {
        let mut r = rng();
        let mut bit_vec = BitSet::<u32>::new();
        b.iter(|| {
            for _ in 0..100 {
    ...
    fn bench_bit_vecset_iter(b: &mut Bencher) {
        let bit_vec = BitSet::<u32>::from_bit_vec(BitVec::from_fn(BENCH_BITS,
                                              |idx| {idx % 3 == 0}));
        b.iter(|| {
            let mut sum = 0;
            for idx in &bit_vec {
